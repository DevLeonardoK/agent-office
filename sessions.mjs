...
export function makeSession(id, cwd, now) {
  return {
    ...
    lastSeen: now,
    closed: false,
    // O servidor não monta cena (ADR-0001): a sala guarda só a lista de eventos,
    // e é dela que o cliente reconstrói o prédio ao chegar.
    events: [],
  };
}
 ...
 * volta a agir é descartada e reconstruída do zero — ninguém herda os eventos da
 * vida anterior.
 */
